...
// assign meshes
		...
		GraphicMeshObjects[i].GetComponent.<Renderer>().material = SectorMaterials[i];
		GraphicMeshObjects[i].GetComponent.<Renderer>().material.SetTexture("_MainTex", Textures[i]);
...
// fill in the remainder of points
	...
	if (IsElevationMap == false) {	
		for (i=0; i<Sectors; i++) {
			...
			GraphicMeshObjects[i].GetComponent.<Renderer>().material = SectorMaterials[i];			// assign shaders
		}
	} else {
		for (i=0; i<Sectors; i++) {
			GraphicMeshes[i] = GraphicMeshObjects[i].GetComponent(MeshFilter).mesh;
...
function SaveTextures() {
	for (n=0; n<Sectors; n++) {
		var tex = GraphicMeshObjects[n].GetComponent.<Renderer>().material.GetTexture("_MainTex");
...
function UpdateMaps() {
	for (i=0; i<Sectors; i++) {
		GraphicMeshObjects[i].GetComponent.<Renderer>().material.GetTexture("_MainTex").Apply();
